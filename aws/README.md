...
Suppose user wants to write the dataframe `df` to the s3 bucket `myBucket` under the path `outputPath` in the `json` format.
Then user need to perform below steps:
...
Suppose user wants to write the dataframe `df` to s3 bucket `myBucket` under the path `outputPath` in the `parquet` format.
Then user need to perform below steps:
